...
本题还有一个故意引入的漏洞。注意到每 tick 价格是成交均价，AS 做市商也是根据上次均价进行报价的。而题目中并没有防止自成交，因此可以利用这个特性进行 wash trading，操控成交均价。不过我测试了感觉效果有限。小范围的操纵效果不好；大范围的操纵可能会被散户割。
...
另外总体来讲 GPT5-Codex 的逻辑能力还是比较强的。例如本题的两个做市商的主要逻辑都是 LLM 写的。其他模型写不出来。
